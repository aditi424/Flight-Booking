...
class Airport(models.Model):
    ...
    def __str__(self):
        return f"{self.airport} ({self.code}) - {self.city}, {self.country}"
...
class Flight(models.Model):
    ...
    class Meta:
        ...
        ordering = ['depart_time']  # Orders flights by departure time

    def __str__(self):
        return f"{self.airline} Flight from {self.origin} to {self.destination} at {self.depart_time}"
...
# class FlightPlace(models.Model):
#     city = models.CharField(max_length=100)
...
#     class Meta:
#         db_table = 'flight_place'  # Specify the exact table name in your database
