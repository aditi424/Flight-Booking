from django.shortcuts import render, HttpResponse, HttpResponseRedirect, get_object_or_404, redirect #redirect #admin
...
#from .models import FlightPlace
...
def admin_register_view(request):
    if request.method == "POST":
        ...
        if password != confirmation:
            return render(request, "flight/adminregister.html", {
                "message": "Passwords must match."
            })
...
def admin_login_view(request):
    if request.method == "POST":
        ...
        user = authenticate(request, username=username, password=password)
        if user is not None and user.is_staff and passkey == "12345":
            login(request, user) 
            return redirect("manage_flights")  
        else:
            return render(request, "flight/adminlogin.html", {
                "message": "Invalid credentials or passkey."
            })
    return render(request, "flight/adminlogin.html")
...
def manage_flights(request):
    ...
    if query:
        flights_list = Flight.objects.filter(plane__icontains=query)  
    else:
    ...
    return render(request, 'flight/manage_flight.html', {'page_obj': page_obj, 'query': query})
...
def edit_flight(request, id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return render(request, 'flight/edit_flight.html', {
                'flight': flight,
                'error_message': "Invalid duration format. Please use 'HH:MM:SS'."
            })


        flight.duration = duration
        flight.plane = request.POST.get('plane')
        flight.airline = request.POST.get('airline')
        

        flight.save()
        return redirect(reverse('manage_flights'))

    return render(request, 'flight/edit_flight.html', {'flight': flight})
...
def admin_dashboard(request):
    ...
    return render(request, 'flight/admindashboard.html', {'flight_place': page_obj})